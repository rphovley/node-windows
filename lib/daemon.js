    ...
    */
    ...
    install: {
      ...
      value: function (dir) {
        ...
        // Check and create the npm directory for the system user if it doesn't exist
        const npmPath = path.join(process.env.SystemDrive, 'Windows', 'system32', 'config', 'systemprofile', 'AppData', 'Roaming', 'npm');
        if (!fs.existsSync(npmPath)) {
          console.log("npm directory does not exist, creating...")
          try {
            fs.mkdirSync(npmPath);
          } catch (err) {
            // Only log error if it's not because directory was created by another process
            if (err.code !== 'EEXIST') {
              this.log.error(`Failed to create npm directory: ${err.message}`);
              this.emit('error', err);
              return;
            }
          }
